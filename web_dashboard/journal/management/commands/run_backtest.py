...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        symbol, days, initial_balance, spread = "XAUUSD", 30, 10000, 0.00

        mt5 = MT5Executor()
        if not mt5.connect(): return

        # Load LTF (1m) and HTF (15m) data
        ...
        agent = MultiTimeframeSFPAgent("SFP_Universal_BT", 8888)
        ...
        with transaction.atomic():
            session = BacktestSession.objects.create(
                agent_name=agent.name, symbol=symbol, timeframe="M1/M15",
                initial_balance=initial_balance, spread_points=int(spread * 100),
                start_date=make_aware(ltf_candles[0].timestamp),
                end_date=make_aware(ltf_candles[-1].timestamp),
                final_balance=broker.balance, net_profit=broker.balance - initial_balance,
                max_drawdown_percent=broker.max_drawdown_percent,
                max_drawdown_amount=broker.max_drawdown_amount,
                win_rate=(len([t for t in broker.closed_history if t['net_profit'] > 0]) / len(
                    broker.closed_history) * 100) if broker.closed_history else 0,
                total_trades=len(broker.closed_history)
            )

            Trade.objects.bulk_create([
                Trade(
                    session=session, ticket=t['ticket'], direction=t['type'].name,
                    entry_price=t['entry_price'], exit_price=t['exit_price'],
                    sl=t.get('sl', 0), tp=t.get('tp', 0), volume=t['volume'],
                    gross_profit=t['gross_profit'], net_profit=t['net_profit'],
                    open_time=make_aware(t['open_time']), close_time=make_aware(t['close_time']),
                    duration_minutes=t['duration'], entry_reason=t['entry_reason'], exit_reason=t['exit_reason']
                ) for t in broker.closed_history
            ])

            EquityPoint.objects.bulk_create([
                EquityPoint(
                    session=session, timestamp=make_aware(p['timestamp']),
                    balance=p['balance'], equity=p['equity'], drawdown_percent=p['dd']
                ) for p in equity_curve[::10]
            ])
        print(f"✅ Success. Trades: {len(broker.closed_history)}")